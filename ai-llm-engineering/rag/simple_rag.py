...
class OllamaEmbeddingFunction:
    def __init__(self, base_url="http://host.docker.internal:11434", model_name="nomic-embed-text"):
    ...
    def __call__(self, input):
        ...
        Returns:
            List of embeddings, one for each input string
        """
        ...
        for text in input:
            try:
                # Using the OpenAI client to make a direct request to Ollama's embeddings endpoint
                ...
                # Fallback: return a zero vector of appropriate dimension (typically 384 for nomic-embed-text)
...
class EmbeddingModel:
    def __init__(self, model_type="openai"):
        ...
        if model_type == "openai":
            ...
            # Use a custom embedding function for Ollama's Nomic model
            self.embedding_fn = OllamaEmbeddingFunction(
                base_url="http://host.docker.internal:11434",
                model_name="nomic-embed-text"
            )
            
            # Alternatively, if you want to use the default embedding function for Chroma
...
def load_csv():
    df = pd.read_csv(directory_path + "space_facts.csv")
